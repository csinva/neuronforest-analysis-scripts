function evaluate_predictions(files, dims, description, root)
...
%    min_step = 0.1;
    ...
    [p_thresholds, p_err, p_tp, p_fp, p_pos, p_neg, p_sqerr] = ...
        evaluate_thresholds(files, dims, initial_thresholds_pixel, 'pixel', min_step);
    
    [r_thresholds, r_fscore, r_tp, r_fp, r_pos, r_neg, ~] = ...
        evaluate_thresholds(files, dims, initial_thresholds_rand, 'rand', min_step);
    ...
    min_threshold_idx_pixel = floor((length(initial_thresholds_pixel)+1)/2);
    max_threshold_idx_pixel = ceil(length(p_thresholds) + 1 - min_threshold_idx_pixel);
    min_threshold_idx_rand = floor((length(initial_thresholds_rand)+1)/2);
    max_threshold_idx_rand = ceil(length(r_thresholds) + 1 - min_threshold_idx_rand);
    ...
    xlabel('Threshold');
    ylabel('Rand F-Score');
    xlim([r_thresholds(min_threshold_idx_rand), r_thresholds(max_threshold_idx_rand)]);
    ylim([0, 1]);
    ...
    xlabel('Threshold');
    ylabel('Pixel Error');
    xlim([p_thresholds(min_threshold_idx_pixel), p_thresholds(max_threshold_idx_pixel)]);
    ylim([0, 1]);
    ...
    saveas(plt, [root '/errors_new.fig']);
    saveas(plt, [root '/errors_new.png'], 'png');
    
    fclose(f);
    
    save([root '/errors_new.mat'], ...
        'r_thresholds', 'r_fscore', 'r_tp', 'r_fp', 'r_pos', 'r_neg', ...
        'p_thresholds', 'p_err', 'p_tp', 'p_fp', 'p_pos', 'p_neg', 'p_sqerr', 'min_threshold_idx_pixel','max_threshold_idx_pixel','min_threshold_idx_rand','max_threshold_idx_rand');
end
